...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static uint8_t Lis3dhAlarmHappenFlag = FALSE;
/* Private function prototypes -----------------------------------------------*/
extern I2C_HandleTypeDef hi2c2;

void SetLis3dhAlarmStatus(uint8_t Status)
{
	Lis3dhAlarmHappenFlag = Status;
}

uint8_t GetLis3dhAlarmStatus(void)
{
	return Lis3dhAlarmHappenFlag;
}

uint8_t Lis3dhCheckWhoAmI(void)
{
	uint8_t WhoAmI;
	
	/* Read WHO AM I register */
	if ( LIS3DH_GetWHO_AM_I(&WhoAmI) == MEMS_ERROR || \
		WhoAmI != WHOAMI_LIS3DH_ACC)
	{
		// Print out
		Sprintf(URT_NRCMD, "\r\n[MEMS] fail id(0x%X)", WhoAmI);
		return FALSE;
	}
	else
	{
...
void Lis3dhDeinit(void)
{
	// Disable Int
	LIS3DH_SetIntConfiguration(LIS3DH_INT1_ZHIE_DISABLE | LIS3DH_INT1_ZLIE_DISABLE |
...
uint8_t LIS3DH_ReadReg(uint8_t Reg, uint8_t* Data) {
  ...
  //Reg |= 0x80; // D7=1 -> read; D6=0 -> single byte
  ...
  uint16_t DevAddr = LIS3DH_MEMS_I2C_ADDRESS&0xFE;
  
  Result = HAL_I2C_Mem_Read(&hi2c2, DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, Data, 1, BLE_I2C_TIMEOUT);

  DelayUsTime(125);
  if(Result != HAL_OK)
  {
	  // Print out
	  Sprintf(URT_NRCMD, "\r\n[IIC] read fail %d", Result);
	  return MEMS_ERROR;
  }
...
uint8_t LIS3DH_WriteReg(uint8_t WriteAddr, uint8_t Data) {
  ...
  //WriteAddr &= 0x3f;  // D7=0 -> write; D6=0 -> single byte
 ...
 //WriteByte(Data);
  ...
  Result = HAL_I2C_Mem_Write(&hi2c2, DevAddr, WriteAddr, I2C_MEMADD_SIZE_8BIT, &Data, 1,BLE_I2C_TIMEOUT);

  if(Result != HAL_OK)
  {
	  Sprintf(URT_NRCMD, "\r\n[IIC]: write fail %d", Result);
	  return MEMS_ERROR;
  }
...
/**
  ...
  * @param LIS3DH_POSITION_6D_t direction
			LIS3DH_UP_SX 	= 0x44,
			LIS3DH_UP_DX 	= 0x42,
			LIS3DH_DW_SX 	= 0x41,
			LIS3DH_DW_DX 	= 0x48,
...
{
	uint8_t response;
	static LIS3DH_POSITION_6D_t NewLis3dhDirection = LIS3DH_UNKOWN;
	static LIS3DH_POSITION_6D_t OldLis3dhDirection = LIS3DH_UNKOWN;

	response = LIS3DH_Get6DPosition(&NewLis3dhDirection);
	if((response == 1) && (OldLis3dhDirection!=NewLis3dhDirection))
	{
	   	switch (NewLis3dhDirection)
	    	{
		    case LIS3DH_UP_SX:   Sprintf(URT_NRCMD,"\r\n[MEMS] Position = UP_SX  [0x%x]",NewLis3dhDirection); 
		    		break;
		    case LIS3DH_UP_DX:    Sprintf(URT_NRCMD,"\r\n[MEMS] Position = UP_DX [0x%x]",NewLis3dhDirection);      
		    		break;
		    case LIS3DH_DW_SX:   Sprintf(URT_NRCMD,"\r\n[MEMS] Position = DW_SX [0x%x]",NewLis3dhDirection);
		    		break;              
		    case LIS3DH_DW_DX:   Sprintf(URT_NRCMD,"\r\n[MEMS] Position = DW_DX [0x%x]",NewLis3dhDirection);   
		    		break; 
		    case LIS3DH_TOP:      Sprintf(URT_NRCMD,"\r\n[MEMS] Position = TOP [0x%x]",NewLis3dhDirection);   
		    		break; 
		    case LIS3DH_BOTTOM:   Sprintf(URT_NRCMD,"\r\n[MEMS] Position = BOTTOM [0x%x]",NewLis3dhDirection);     
		    		break; 
		    default:       		Sprintf(URT_NRCMD,"\r\n[MEMS] Position = unknown [0x%x]",NewLis3dhDirection);   
		    		break;
	    	}
		OldLis3dhDirection = NewLis3dhDirection;
	}
	
	// Print Out
	Sprintf(URT_NRCMD,"\r\n[MEMS] Position = [0x%x]",NewLis3dhDirection);
...
void Lis3dhIntProcess(void)
{
	...
	if(GetLis3dhAlarmStatus() == TRUE)
	{
		...
		// Print out
		Sprintf(URT_NRCMD,"\r\n[MEMS] Interupt Occured");
		// Int process
		Lis3dh_6DPosition_Handle();
		// Reset Int
		LIS3DH_ResetInt1Latch();
	}
}
